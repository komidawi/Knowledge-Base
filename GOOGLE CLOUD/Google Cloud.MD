...
- Also that code is packaged along with its dependencies
- Containers don't boot and entire VM - there are only processes started, so it's quick to run up or down
...
- Each item managed by K8s is represented by an object, which can be viewed an its attributes and state can be changed
...
- kube-scheduler
  ...
  - Analyzes requirements of each Pod and finds the most suitable Node
    - Requirements are like software, hardware, policy, affinity (Pods ond the same Node), anti-affinity (no Pods on the same Node)
